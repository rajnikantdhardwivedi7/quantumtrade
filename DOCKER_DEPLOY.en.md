...
This guide will help you quickly deploy the QuantumTrade OS using Docker.
...
```bash
# Check health status
docker inspect quantumtrade-backend | jq '.[0].State.Health'
docker inspect quantumtrade-frontend | jq '.[0].State.Health'
...
```nginx
# /etc/nginx/sites-available/quantumtrade
server {
    listen 80;
...
# Deploy stack
docker stack deploy -c docker-compose.yml quantumtrade

# View service status
docker stack services quantumtrade

# Scale services
docker service scale quantumtrade_backend=3
```
...
🎉 Congratulations! You've successfully deployed the QuantumTrade OS!
