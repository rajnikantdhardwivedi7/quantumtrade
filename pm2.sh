...
print_header() {
    echo -e "${PURPLE}═══════════════════════════════════════${NC}"
    echo -e "${PURPLE}  🤖 QuantumTrade OS - PM2 Manager${NC}"
    echo -e "${PURPLE}═══════════════════════════════════════${NC}"
    echo ""
}
...
show_status() {
    ...
    echo ""
    print_info "详细信息:"
    pm2 info quantumtrade-backend
    echo ""
    pm2 info quantumtrade-frontend
}
...
show_logs() {
    if [ -z "$2" ]; then
        ...
        pm2 logs quantumtrade-backend
    elif [ "$2" = "frontend" ]; then
        pm2 logs quantumtrade-frontend
    else
...
rebuild_and_restart() {
    ...
    print_info "正在重启后端服务..."
    pm2 restart quantumtrade-backend
